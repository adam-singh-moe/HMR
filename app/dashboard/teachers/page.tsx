...
export default function TeachersPage() {
  ...
  // Check permissions
  const canViewAll = hasPermission("teacher_details.view_all")
  const canViewRegional = hasPermission("teacher_details.view_regional")
  const canViewTeachers = canViewAll || canViewRegional
  ...
  useEffect(() => {
    if (canViewTeachers && !isLoadingPermissions) {
      fetchSchools()
    }
  }, [canViewTeachers, isLoadingPermissions])
      ...
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        <div className="p-4 rounded-xl bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 max-w-md text-center">
          ...
          <p className="text-sm text-red-600 dark:text-red-300">
            You do not have permission to view teacher details.
          </p>
        </div>
      </div>
    )
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
            View teacher details across {canViewRegional && !canViewAll ? 'your region\'s ' : ''}schools
          </p>
        </div>
      </div>
