...
import { Loader2, Download, X, RefreshCw, ChevronRight, Activity, ShieldAlert } from "lucide-react"
...
export default function PEReportsPage() {
  const { user } = useAuth()
  ...
  const [selectedRegionId, setSelectedRegionId] = useState(peReportsCache?.regionId || "")
  ...
  // Check permissions
  const canViewAll = hasPermission("pe_reports.view_all")
  const canViewRegional = hasPermission("pe_reports.view_regional")
  ...
  // Load regions on mount (only if user can view all)
  useEffect(() => {
    if (!canViewAll) {
  ...
  const isCacheValid = useCallback(() => {
    ...
    const filtersMatch =
      ...
      peReportsCache.regionId === selectedRegionId &&
  ...
  // Initial load - only fetch if no valid cache exists and has access
  useEffect(() => {
    if (permissionsLoading || !hasAccess) return
  ...
  useEffect(() => {
    ...
    fetchReports()
  }, [debouncedSearchTerm, selectedRegionId, selectedMonth, selectedYear, currentPage, pageSize, permissionsLoading, hasAccess])
          ...
          <CardContent className="p-6">
            <div className="text-center py-12">
              ...
              <p className="text-slate-500 dark:text-slate-400">
                You do not have permission to view Physical Education Reports.
              </p>
    ...
    <div className="space-y-4 lg:space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">
          {canViewAll
            ? "View PE activities and challenges across all regions"
            : `View PE activities and challenges for ${user?.region_name || 'your region'}`}
        </p>
      </div>
      ...
      <div className="p-4 rounded-xl border border-slate-200/80 dark:border-slate-700/50 bg-white dark:bg-[hsl(222,47%,9%)]">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 flex-wrap">
          ...
          {/* Region Filter - only show if user can view all */}
          {canViewAll ? (
            ...
            /* Show region name as read-only for regional users */
            <Input
