...
import { Loader2, Sparkles, Brain, TrendingUp, AlertCircle, Lightbulb, FileText, Download, Send, RotateCcw, Clock, Copy, Check, ShieldX } from "lucide-react"
...
function RegionalAIInsightsContent() {
  ...
  // Check for either general AI insights view OR regional-specific view permission
  const canViewAll = hasPermission("ai_insights.View")
  const canViewRegional = hasPermission("ai_insights.view_regional")
  const hasAIAccess = canViewAll || canViewRegional
  ...
  // Show access denied if user doesn't have any AI insights permission
  if (!hasAIAccess) {
    return (
      <div className="h-[calc(100vh-120px)] flex items-center justify-center">
