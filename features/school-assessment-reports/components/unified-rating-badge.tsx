...
export const TAPS_GRADE_COMPACT_LABELS: Record<TAPSRatingGrade, string> = {
  'A': 'Outstanding',
  'B': 'High Achieving',
  'C': 'Standard',
  'D': 'Struggling',
...
export const RATING_COMPACT_LABELS: Record<RatingLevel, string> = {
  'outstanding': 'Outstanding',
  'very_good': 'Very Good',
  'good': 'Good',
  'satisfactory': 'Satisfactory',
  'needs_improvement': 'Needs Improv.',
}
...
export function UnifiedRatingBadge({
  ...
  if (isTAPS && tapsRatingGrade) {
    ...
    label = compact ? TAPS_GRADE_COMPACT_LABELS[grade] : TAPS_GRADE_DISPLAY_LABELS[grade] || `Grade ${grade}`;
  ...
  if (!grade && ratingLevel) {
    ...
    if (possibleGrade) {
      grade = possibleGrade;
      label = compact ? RATING_COMPACT_LABELS[normalizedLevel] : RATING_DISPLAY_LABELS[normalizedLevel] || ratingLevel;
  ...
  if (!grade && hasScore) {
    if (isTAPS) {
      ...
      if (levelEntry) {
        ...
        label = compact ? RATING_COMPACT_LABELS[key] : RATING_DISPLAY_LABELS[key] || levelEntry[1].label;
  ...
  return (
    <div className={`flex items-center gap-1.5 ${className}`}>
      <div className={`${compact ? 'h-5 w-5 text-[9px]' : 'h-6 w-6 text-[10px]'} rounded-md flex items-center justify-center font-bold shrink-0 ${colorClass}`}>
        {grade}
      </div>
      {showLabel && (
        <Badge
          variant="outline"
          className={`${colorClass} border font-medium whitespace-nowrap ${compact ? 'text-[10px] px-1.5 py-0' : 'text-xs'}`}
        >
          {label}
        </Badge>
      )}
