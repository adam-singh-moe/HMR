    ...
    <div className="w-full max-w-6xl mx-auto">
      ...
      <div className="text-center mb-8">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-gradient-to-r from-blue-500/10 to-purple-500/10 border border-blue-500/20 mb-4">
          ...
          <span className="text-sm font-medium text-blue-300">Top Performing Schools</span>
        </div>
        <h2 className="text-3xl md:text-4xl font-bold bg-gradient-to-r from-white via-blue-100 to-cyan-200 bg-clip-text text-transparent mb-2">
      ...
      <div className="flex items-center justify-center gap-4 mb-8">
        <Button
          ...
          size="icon"
          onClick={goToPrev}
          className="h-10 w-10 rounded-full bg-slate-800/50 hover:bg-slate-700/50 border border-slate-700/50"
        >
          <ChevronLeft className="h-5 w-5 text-slate-300" />
        </Button>

        <div className="flex items-center gap-3 px-6 py-3 rounded-2xl bg-gradient-to-r from-slate-800/80 to-slate-900/80 border border-slate-700/50 backdrop-blur-sm">
      ...
      >
        {schools.map((school, index) => {
          ...
          return (
            <Card
              key={school.id}
              className={`relative overflow-hidden bg-gradient-to-br from-slate-800/90 to-slate-900/90 border-slate-700/50 backdrop-blur-sm hover:border-slate-600/50 transition-all duration-300 hover:scale-[1.02] hover:shadow-xl hover:shadow-blue-500/10 ${
                index === 0 ? 'md:scale-105 ring-2 ring-yellow-500/30' : ''
              }`}
            >
              {/* Rank Badge */}
              <div className="absolute top-4 left-4 z-10">
                <div className={`flex items-center justify-center w-10 h-10 rounded-full ${
                  ...
                  <RankIcon className={`h-5 w-5 ${index === 0 ? 'text-yellow-900' : index === 1 ? 'text-slate-700' : 'text-amber-200'}`} />
                </div>
              </div>
              ...
              <CardContent className="pt-16 pb-6">
                {/* School Name */}
                <h3 className="text-lg font-bold text-white mb-2 pr-12">{school.name}</h3>

                {/* School Level */}
                <div className="flex items-center gap-2 mb-4">
                  <GraduationCap className="h-4 w-4 text-slate-400" />
                  <span className="text-sm text-slate-400">{school.level} School</span>
                </div>
                ...
                <div className="flex items-end justify-between mt-4 pt-4 border-t border-slate-700/50">
                  <div>
                    ...
                    <div className="flex items-baseline gap-1">
                      <span className="text-3xl font-black text-white">{school.score}</span>
                  ...
                  <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                    ...
                    <span className="text-xs font-medium text-emerald-400">{school.trend}</span>
                  </div>
                </div>
                ...
                <div className="mt-4">
                  <div className="h-2 bg-slate-700/50 rounded-full overflow-hidden">
                    <div
                      className={`h-full bg-gradient-to-r ${ratingGradient} rounded-full transition-all duration-500`}
      ...
      <div className="text-center mt-8">
        <Button
          variant="outline"
          className="border-slate-700 bg-slate-800/50 hover:bg-slate-700/50 text-slate-300 hover:text-white"
        >
